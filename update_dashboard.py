...
REPO = "3529083364chatgpt-debug/ai-hot-daily"
...
def upload(html, retries=5):
    ...
    body = json.dumps({
        "message": "Daily auto-update: append today's AI HOT digest",
...
<body>
  <header class="hero"><div class="wrap">
    <div class="eyebrow">AI HOT · 每日 / 每周 / 每月 AI 晨报</div>
    <h1 id="hTitle">—</h1>
...
<script>
const DB = __DB__;
const SECTIONS = ["模型发布/更新","产品发布/更新","行业动态","论文研究","技巧与观点"];
const ICON = {"模型发布/更新":"🧠","产品发布/更新":"🚀","行业动态":"🌐","论文研究":"📄","技巧与观点":"💡"};
const ACCENT = {"模型发布/更新":"#4f46e5","产品发布/更新":"#059669","行业动态":"#d97706","论文研究":"#e11d48","技巧与观点":"#0284c7"};
...
function render(){
  ...
  document.getElementById("footSrc").textContent="aihot.virxact.com";
...
def main():
    ...
    # fetch dailies list
    lst = json.loads(http_get("https://aihot.virxact.com/api/public/dailies?take=90"))
    dates_in_list = [it["date"] for it in lst.get("items", [])]
    print("[2] dailies archive returned:", len(dates_in_list), "dates")

    today = date.today().strftime("%Y-%m-%d")
    candidates = dates_in_list + ([today] if today not in dates_in_list else [])
    # newest first for fresher data
    ...
    for d in candidates:
        if d in db["days"]:
            continue
        try:
            raw = json.loads(http_get(f"https://aihot.virxact.com/api/public/daily/{d}"))
            _, slim = slim_day(raw)
            db["days"][d] = slim
            added += 1
            print("    + added", d)
        except Exception as e:
            print("    - skip", d, repr(e)[:80])
